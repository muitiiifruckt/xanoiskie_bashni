...
namespace Ханойские_башни.Custom
{
    internal class CustomPicturebox
    {
    }


namespace CodeeloUI.Controls
        //как выяснилось поменять цвет рамки picturebox  очень трудоемко, поэтому я наешл готовый код 
        // в котором человек реализовал кастомный picturebox, в котором немного другие названмия
        // суть в том что мне было очень важно цвет рамки поменять на черный 
    {
        [ToolboxBitmap(typeof(CustomPictureBox), "Icons.CodeeloPictureBox.bmp")]
        [ToolboxItem(true)]
        ...
        public class CustomPictureBox : PictureBox
        {
            ...
            private Color _borderColorFirst = Color.DodgerBlue;
            private Color _borderColorSecond = Color.Tomato;
            ...
            protected override void OnPaint(PaintEventArgs e)
            {
                ...
                var rectContourSmooth =  contourOn;
                var rectBorder = rectContourSmooth;
            ...
            private Bitmap GetCanvasImage()
            {
                ...
                using (var graphics = System.Drawing.Graphics.FromImage(canvasImage))
                {
                    if (Image == null)
                    {
                        graphics.FillRectangle(new SolidBrush(_borderColorFirst), ClientRectangle);
                    }
                    else
                    {
